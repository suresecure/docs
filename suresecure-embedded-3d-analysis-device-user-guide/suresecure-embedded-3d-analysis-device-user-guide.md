...
#设备简介
嵌入式3D行为分析设备采用3D环境探测技术，能够对监控场景的3D信息进行探测和建模，进而对场景内的人和物进行准确的检测和跟踪，动态计算场景内人和物的物理尺寸、位置、速度、相对位置等信息。以这些信息为基础，嵌入式3D行为分析设备针对金融安防领域的三大场景（ATM厅、ATM防护舱以及加钞间）设计了专门的智能预警策略，检测不同场景中出现的异常情况并报警，提示监控中心关注可能发生的异常行为，预防违法犯罪事件的发生。

嵌入式3D行为分析设备能够对监控场景内出现的如下情况进行分析和报警：
人物进入和离开、徘徊、滞留、倒地、下蹲、速度异常、一米区违规、尾随、推搡、物品滞留、越过拌线、违规加钞、箱门打开等等。
#安装部署

戍融智能嵌入式3D行为分析设备的完整包装包含如下：
...
* **物品滞留检测功能**
	* **报警项目：**物品滞留
	* **功能说明：**客户进入监控区域，在监控范围内客户将有一定体积的物品（厚度高于15cm，长宽均大于10cm）放入场景内，并离开该物体超过一定时间，系统会立即触发“物体滞留”报警。
	* **可调参数：**
		* *滞留物阈值：*单位为秒，其含义为：人员进入后将某物品放下，再远离该物品，超过多少秒会触发报警。
...
* **异常行为检测**
	* **报警项目**：异常行为
	* **功能说明**：客户1人进入监控区域，在监控范围的有效区域内进行数次大幅度动作，系统会立即触发“异常行为”报警。
	* **可调参数**：
		* *异常行为阈值：判定客户进行大幅度动作的阈值，越大越不灵敏。
		* *异常挥动次数：客户进行大幅度动作的次数，超过阈值触发“异常行为”报警。
	* **相关区域设置**：布防区域
* **人员徘徊报警**
	* **报警项目**：疑似徘徊
	* **功能说明**：客户1人进入监控区域，在监控范围的有效区域内客户移动的距离超过设定参数，系统会立即触发“有人徘徊”报警。
	* **可调参数**：
		* *徘徊阈值：*单位为米，其含义为：布防区域内人员累计移动距离超过该值则触发报警。
	* **相关区域设置**：布防区域
* **尾随检测功能**
	* **报警项目**：疑似尾随
	* **功能说明**：客户2人进入监控区域，一人进入一米区内办理业务，另一人随后跟进，进入一米区，系统会立即触发“有人尾随”报警。
	* **可调参数**：
		* *尾随间隔下限：*单位为秒，其含义为：一人进入一米区后，另一人必须在多少秒后进入一米区，才会触发报警。
		* *尾随间隔上限：*单位为秒，其含义为：一人进入一米区后，另一人必须在多少秒前进入一米区，才会触发报警。
	* **相关区域设置**：布防区域，一米区
* **人员进入检测功能**
	* **报警项目**：有人进入防护区
...
* **违规假钞检测功能**
	* **报警项目**：违规加钞
	* **功能说明**：加钞员有且仅有1人进入监控区域的加钞区后，系统会立即触发“违规加钞”报警。如果设置加超区的人数上限，则进入监控区域的人数超过该上限也会报警。
	* **可调参数**：
		* **加超区人数上限：**默认值为0，如果设为大于0的值，则有人进入加钞区时，布防区域内人数超过该值也会报警。 
	* **相关区域设置**：加钞区
